...
const server = createServer(async (request, response) => {
  ...
  if (request.method === "POST" && url.pathname === "/api/mobile/sales") {
    try {
      ...
      body.employeeName = session.username;
  ...
  if (request.method === "POST" && url.pathname === "/api/mobile/shift/open") {
    try {
      const session = readRequestSession(request);
      if (!session || session.role !== "employee") return sendJsonError(response, 403, "Войдите как сотрудник");
...
function publicSession(user) {
  const employeeKind = user.employee_kind || (user.role === "admin" ? "admin" : "pavilion");
  return {
    ok: true,
    role: user.role,
    employeeName: user.username,
    employeeKind,
    saleChannel: employeeKind === "warehouse" ? "warehouse" : "pavilion",
...
function buildClientRows(sales, writeoffs) {
  ...
  for (const row of sales) {
    ...
    const employeeName = String(row.employee_name ?? "").trim() || "Старые данные";
    const key = `${String(row.sale_channel ?? "pavilion")}__${name}__${paymentKind(row)}__${coolerStatusFromRow(row)}__${employeeName}`;
    groups[key] ??= {
      name,
      employeeName,
  ...
  for (const row of writeoffs) {
    ...
    groups[key] ??= {
      name,
      employeeName: String(row.employee_name ?? "").trim() || "Склад",
      channel: "warehouse",
      paymentType: "",
      coolerStatus: "",
      sold: 0,
...
function sendJsonError(response, status, error) {
  send(response, status, "application/json; charset=utf-8", JSON.stringify({ error }));
}
...
<head>
  ...
  <style>
    ...
    *{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif!important}
    ...
    .auth-mode{display:grid;grid-template-columns:1fr 1fr;gap:7px}.auth-mode button{height:42px;border:1px solid rgba(130,160,205,.22);border-radius:14px;background:#111c2d;color:#a8b5c9;font-weight:850}.auth-mode button.active{background:#3f7cff;color:#fff;border-color:#76a8ff}.auth-note{text-align:center}
    /* Graphite grid theme */
    :root{--ink:#f7f7f7;--muted:#939393;--line:rgba(255,255,255,.12);--aqua:#286bff;--soft:#171717;--danger:#ff6b7d;--panel:#0b0b0b;--panel2:#111;--glow:#7ca7ff}
    body,body.logged-out{background:#050505!important;color:var(--ink)!important}
    body:before,.logged-out:before{opacity:1!important;filter:none!important;background-image:linear-gradient(rgba(255,255,255,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.035) 1px,transparent 1px)!important;background-size:18px 18px!important;background-position:center top!important}
    body:after{background:radial-gradient(circle at 50% 18%,rgba(46,104,255,.16),transparent 32%),linear-gradient(180deg,transparent 55%,rgba(0,0,0,.52))!important}
    header{padding:0 4px!important}.logo{background:#111!important;border:1px solid rgba(255,255,255,.14)!important;border-radius:999px!important;box-shadow:none!important}.logo img{border-radius:999px!important}.pill,.shift-pill,.channel-badge{background:#161616!important;border:1px solid rgba(255,255,255,.13)!important;color:#f1f1f1!important;border-radius:999px!important}
    .card{background:rgba(10,10,10,.96)!important;border:1px solid rgba(255,255,255,.12)!important;border-radius:24px!important;box-shadow:0 22px 54px rgba(0,0,0,.34)!important}
    .sum,.report-box,.closed-summary,.work-kpi div,.profit-card,.warehouse-debt,.report-mini div,.asset-card,.asset-icon,.recent-orders{background:#141414!important;border:1px solid rgba(255,255,255,.09)!important;border-radius:20px!important}
    input,textarea{background:#161616!important;border:1px solid rgba(255,255,255,.10)!important;color:#fff!important;border-radius:999px!important;box-shadow:none!important}textarea{border-radius:22px!important}input:focus,textarea:focus{border-color:#5f8fff!important;box-shadow:0 0 0 3px rgba(45,103,255,.16)!important}input::placeholder,textarea::placeholder{color:#656565!important}
    .prices,.payments{background:#111!important;border:1px solid rgba(255,255,255,.08)!important;border-radius:999px!important}.price,.payment,.expense-btn,.cooler-btn,.mini-btn,.report-payments button,.report-scope button,.auth-mode button,.reset-btn{background:#191919!important;border:1px solid rgba(255,255,255,.10)!important;color:#d7d7d7!important;border-radius:999px!important;box-shadow:none!important}
    .price.active,.payment.active,.expense-btn.active,.report-payments button.active,.report-scope button.active,.auth-mode button.active{background:radial-gradient(circle at 78% 50%,#83a9ff 0%,#286bff 38%,#13265b 100%)!important;border-color:#5e8fff!important;color:#fff!important;box-shadow:0 0 24px rgba(40,107,255,.32)!important}
    .submit,.logout-btn,.close-btn{background:#f6f6f6!important;border:1px solid #fff!important;color:#090909!important;border-radius:999px!important;box-shadow:none!important}.submit:active,.logout-btn:active,.close-btn:active{background:#286bff!important;border-color:#286bff!important;color:#fff!important}.submit:disabled{background:#2a2a2a!important;border-color:#333!important;color:#858585!important}
    .footer-inner{background:rgba(12,12,12,.96)!important;border:1px solid rgba(255,255,255,.14)!important;border-radius:999px!important;box-shadow:0 18px 46px rgba(0,0,0,.50)!important}.footer button{background:transparent!important;color:#777!important;border-radius:999px!important}.footer button.active{background:#f4f4f4!important;color:#111!important;border:1px solid #fff!important;box-shadow:none!important}.footer button svg{stroke-width:1.8!important}
    .report-row{background:#151515!important;border-color:rgba(255,255,255,.08)!important;border-radius:18px!important}.report-row.head{background:#0e0e0e!important;color:#8daeff!important}.client-table .method{background:#242424!important;border-color:rgba(255,255,255,.10)!important}.report-section{color:#a9a9a9!important;border-color:rgba(255,255,255,.10)!important}.report-total b,.paid{color:#fff!important}
    .ok{background:rgba(34,168,112,.13)!important;color:#7ee1b6!important;border-color:rgba(65,211,150,.24)!important}.err{background:rgba(255,84,108,.12)!important;color:#ff8fa0!important;border-color:rgba(255,107,125,.24)!important}
    .cooler-btn.our.active{background:radial-gradient(circle at 75% 40%,#8df0cb,#28aa7b 55%,#174f3e)!important;color:#fff!important;border-color:#66d6ad!important}.cooler-btn.not.active{background:radial-gradient(circle at 75% 40%,#ff9bac,#d9415d 55%,#651d2b)!important;color:#fff!important;border-color:#ee7187!important}
    .asset-line{background:#202020!important;border-color:rgba(255,255,255,.10)!important}.asset-line-warehouse{background:#f5f5f5!important}.asset-line-client{background:#286bff!important}.asset-line-writeoff{background:#d94562!important}
    .logged-out #homePage>.panel{background:transparent!important}.logged-out .home-splash-brand img{filter:grayscale(1) brightness(1.8) drop-shadow(0 20px 42px rgba(0,0,0,.55))!important}.logged-out #pinBlock{background:rgba(9,9,9,.96)!important;border:1px solid rgba(255,255,255,.14)!important;border-radius:28px!important}.logged-out #showLoginButton,.logged-out #loginButton{background:#f7f7f7!important;color:#080808!important;border:1px solid #fff!important}.logged-out .pin-grid input{background:#181818!important;border-color:rgba(255,255,255,.12)!important}.auth-mode{padding:4px;background:#111;border:1px solid rgba(255,255,255,.08);border-radius:999px}.auth-mode button{border:0!important}.auth-mode button.active{background:#f4f4f4!important;color:#111!important;box-shadow:none!important}
    .boot-loader{background:#050505!important;background-image:linear-gradient(rgba(255,255,255,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.035) 1px,transparent 1px)!important;background-size:18px 18px!important}.loader-ring{border-color:rgba(255,255,255,.14)!important;border-top-color:#fff!important}
    /* Editorial light theme based on the supplied mobile reference */
    :root{--ink:#111;--muted:#777;--line:rgba(17,17,17,.08);--aqua:#1769ff;--soft:#efefed;--danger:#d9304f;--panel:#f7f7f5;--panel2:#fff;--glow:#7da4ff}
    body{background:#e8e8e6!important;color:#111!important}body:before,body:after{display:none!important}
    .app{background:#f5f5f3;border-radius:34px;box-shadow:0 26px 70px rgba(0,0,0,.14);padding:18px 14px 12px}
    header{padding:2px 4px!important}.logo{background:#fff!important;border:1px solid rgba(0,0,0,.08)!important;box-shadow:none!important}.pill,.shift-pill,.channel-badge{background:#ececea!important;border:1px solid rgba(0,0,0,.07)!important;color:#333!important}.sub{color:#777!important}h1{color:#111!important}
    .card{background:#fff!important;border:0!important;border-radius:25px!important;box-shadow:0 1px 0 rgba(0,0,0,.04),0 12px 30px rgba(0,0,0,.055)!important}
    .sum,.report-box,.closed-summary,.work-kpi div,.profit-card,.warehouse-debt,.report-mini div,.asset-card,.asset-icon,.recent-orders{background:#f1f1ef!important;border:0!important;border-radius:20px!important;color:#111!important}
    label span,.row span:first-child,.hint,.fixed-list,.report-section,.asset-title,.asset-sub,.work-kpi span,.report-row small{color:#737373!important}
    input,textarea{background:#efefed!important;border:0!important;color:#111!important;border-radius:999px!important;box-shadow:none!important}textarea{border-radius:22px!important}input:focus,textarea:focus{box-shadow:0 0 0 2px #1769ff!important}input::placeholder,textarea::placeholder{color:#9a9a98!important}
    .prices,.payments{background:#ececea!important;border:0!important;border-radius:999px!important}.price,.payment,.expense-btn,.cooler-btn,.mini-btn,.report-payments button,.report-scope button,.auth-mode button,.reset-btn{background:#e9e9e7!important;border:0!important;color:#444!important;border-radius:999px!important;box-shadow:none!important}
    .price.active,.payment.active,.expense-btn.active,.report-payments button.active,.report-scope button.active{background:radial-gradient(circle at 78% 44%,#a8bfff 0%,#3678ff 42%,#1746b6 100%)!important;color:#fff!important;box-shadow:0 10px 24px rgba(38,102,255,.23)!important}
    .submit,.logout-btn,.close-btn{position:relative;background:#111!important;border:0!important;color:#fff!important;border-radius:999px!important;box-shadow:none!important}.submit:disabled{background:#c7c7c5!important;color:#777!important}.logout-btn{background:#111!important;color:#fff!important}.reset-btn{background:#ececea!important;color:#b62943!important}
    .footer-inner{background:#111!important;border:0!important;box-shadow:0 14px 32px rgba(0,0,0,.24)!important}.footer button{background:transparent!important;color:#777!important}.footer button.active{background:radial-gradient(circle at 68% 30%,#92b0ff,#286fff 52%,#1646bc)!important;color:#fff!important;border:0!important;box-shadow:0 9px 22px rgba(40,111,255,.34)!important}.footer button svg{stroke-width:1.7!important}
    .report-row{background:#fff!important;border:0!important;color:#111!important}.report-row.head{background:#e9e9e7!important;color:#555!important}.client-table .method{background:#e5e5e3!important;border:0!important;color:#222!important}.report-section{border-color:rgba(0,0,0,.08)!important}.report-total b,.paid,.total,.asset-value,.report-mini b{color:#111!important}
    .ok{background:#def5ea!important;color:#15724e!important;border:0!important}.err{background:#ffe4e9!important;color:#ae2940!important;border:0!important}
    .cooler-btn.our.active{background:radial-gradient(circle at 70% 32%,#aef0da,#31b881 58%,#16845b)!important;color:#fff!important;border:0!important}.cooler-btn.not.active{background:radial-gradient(circle at 70% 32%,#ffb3bf,#df4b66 58%,#a72840)!important;color:#fff!important;border:0!important}
    .asset-line{background:#dededb!important;border:0!important}.asset-line-warehouse{background:#111!important}.asset-line-client{background:#286fff!important}.asset-line-writeoff{background:#df4b66!important}
    .auth-mode{background:#e9e9e7!important;border:0!important}.auth-mode button.active{background:#111!important;color:#fff!important}.auth-mode button{color:#555!important}
    .logged-out{background:radial-gradient(circle at 66% 16%,#b9caff 0%,#4d83ff 26%,#0b46d0 46%,#050505 76%)!important}.logged-out:before,.logged-out:after{display:none!important}.logged-out .app{background:transparent!important;border-radius:0;box-shadow:none;padding:0}.logged-out #homePage>.panel{background:transparent!important}.logged-out .home-splash-brand img{filter:brightness(0) invert(1) drop-shadow(0 18px 36px rgba(0,0,0,.28))!important}.logged-out #pinBlock{background:rgba(245,245,243,.96)!important;border:0!important;color:#111!important;box-shadow:0 22px 54px rgba(0,0,0,.22)!important}.logged-out #pinBlock label span,.logged-out #authNote{color:#666!important}.logged-out .pin-grid input,.logged-out #loginUsername{background:#e8e8e6!important;color:#111!important;border:0!important}.logged-out #showLoginButton,.logged-out #loginButton{background:#111!important;color:#fff!important;border:0!important}
    .boot-loader{background:#f5f5f3!important;background-image:none!important}.loader-ring{border-color:rgba(0,0,0,.10)!important;border-top-color:#1769ff!important}
    .price.active,.payment.active,.expense-btn.active,.report-payments button.active,.report-scope button.active{background:#111!important;color:#fff!important;box-shadow:none!important}
    .footer button.active{background:#f4f4f2!important;color:#111!important;border:0!important;box-shadow:none!important}
    @media (max-width:430px){body{background:#f5f5f3!important}.app{min-height:calc(100dvh - var(--device-top) - var(--content-bottom))!important;border-radius:0;box-shadow:none;padding:10px 4px 0}.logged-out .app{padding:0}}
    /* Neon smart-home theme experiment */
    :root{--ink:#fff;--muted:#aaa3b8;--line:rgba(255,255,255,.08);--aqua:#b8ff42;--soft:#211b2e;--danger:#ff6f91;--panel:#100b18;--panel2:#1b1427;--glow:#d478ff}
    body{background:radial-gradient(circle at 82% 8%,rgba(180,92,255,.30),transparent 30%),radial-gradient(circle at 8% 88%,rgba(116,57,211,.24),transparent 34%),#08050e!important;color:#fff!important}
    .app{background:#0d0914!important;border:1px solid rgba(215,172,255,.14);box-shadow:0 28px 78px rgba(19,6,36,.46),0 0 55px rgba(147,69,255,.12)!important}
    header .logo{background:#1d152a!important;border-color:rgba(216,175,255,.18)!important}.pill,.shift-pill,.channel-badge{background:#211a2f!important;border-color:rgba(255,255,255,.08)!important;color:#eee9f5!important}.sub{color:#9d95aa!important}h1{color:#fff!important}
    .card{background:linear-gradient(155deg,#1b1427,#100b18 72%)!important;border:1px solid rgba(226,197,255,.10)!important;box-shadow:0 18px 46px rgba(0,0,0,.30)!important}
    .sum,.report-box,.closed-summary,.work-kpi div,.profit-card,.warehouse-debt,.report-mini div,.asset-card,.asset-icon,.recent-orders{background:#211a2e!important;border:1px solid rgba(255,255,255,.06)!important;color:#fff!important}
    label span,.row span:first-child,.hint,.fixed-list,.report-section,.asset-title,.asset-sub,.work-kpi span,.report-row small{color:#aaa3b8!important}
    input,textarea{background:#201929!important;border:1px solid rgba(255,255,255,.07)!important;color:#fff!important}input::placeholder,textarea::placeholder{color:#70697b!important}input:focus,textarea:focus{box-shadow:0 0 0 2px #b8ff42!important;border-color:#b8ff42!important}
    .prices,.payments{background:#171120!important}.price,.payment,.expense-btn,.cooler-btn,.mini-btn,.report-payments button,.report-scope button,.auth-mode button,.reset-btn{background:#282034!important;color:#d8d2df!important;border:1px solid rgba(255,255,255,.06)!important}
    .price.active,.payment.active,.expense-btn.active,.report-payments button.active,.report-scope button.active{background:#b8ff42!important;color:#101508!important;border-color:#b8ff42!important;box-shadow:0 0 24px rgba(184,255,66,.20)!important}
    .submit,.logout-btn,.close-btn{background:#b8ff42!important;color:#101508!important;border:0!important;box-shadow:0 10px 28px rgba(184,255,66,.13)!important}.submit:disabled{background:#393143!important;color:#837b8d!important}.reset-btn{background:#2a1b2d!important;color:#ff8ba8!important}
    .footer-inner{background:#100b18!important;border:1px solid rgba(226,197,255,.10)!important;box-shadow:0 18px 46px rgba(0,0,0,.48)!important}.footer button{color:#777080!important}.footer button.active{background:#b8ff42!important;color:#11170a!important;box-shadow:0 0 24px rgba(184,255,66,.24)!important}
    .report-row{background:#211a2e!important;color:#fff!important}.report-row.head{background:#171120!important;color:#c8bfce!important}.client-table .method{background:#30283a!important;color:#fff!important}.report-total b,.paid,.total,.asset-value,.report-mini b{color:#fff!important}.report-section{border-color:rgba(255,255,255,.08)!important}
    .ok{background:rgba(184,255,66,.12)!important;color:#ceff82!important}.err{background:rgba(255,81,131,.14)!important;color:#ff9bb8!important}
    .asset-line{background:#30283a!important}.asset-line-warehouse{background:#b8ff42!important}.asset-line-client{background:#cb67ff!important}.asset-line-writeoff{background:#ff5f8b!important}
    .auth-mode{background:#171120!important}.auth-mode button{color:#aaa3b8!important}.auth-mode button.active{background:#b8ff42!important;color:#101508!important}
    .logged-out{background:radial-gradient(ellipse at 42% 34%,rgba(238,210,255,.92) 0%,rgba(193,106,255,.75) 7%,rgba(105,39,197,.42) 18%,transparent 38%),radial-gradient(ellipse at 72% 72%,rgba(190,105,255,.68),rgba(90,32,176,.28) 22%,transparent 46%),#07040c!important}.logged-out .app{background:transparent!important;border:0!important;box-shadow:none!important}.logged-out .home-splash-brand img{filter:brightness(0) invert(1) drop-shadow(0 20px 42px rgba(0,0,0,.42))!important}.logged-out #pinBlock{background:rgba(20,13,29,.94)!important;color:#fff!important;box-shadow:0 24px 64px rgba(0,0,0,.40)!important}.logged-out #pinBlock label span,.logged-out #authNote{color:#aaa3b8!important}.logged-out .pin-grid input,.logged-out #loginUsername{background:#2a2136!important;color:#fff!important}.logged-out #showLoginButton,.logged-out #loginButton{background:#b8ff42!important;color:#101508!important}
    .boot-loader{background:#08050e!important}.loader-ring{border-color:rgba(255,255,255,.12)!important;border-top-color:#b8ff42!important}
    @media (max-width:430px){body{background:#0d0914!important}.app{border:0!important;box-shadow:none!important}}
  </style>
</head>
<body>
...
$("logoutButton").onclick=()=>{
  ...
  localStorage.removeItem("waterOpsEmployee");
  localStorage.removeItem("waterOpsEmployeeKind");
  localStorage.removeItem("waterOpsAuthToken");
  localStorage.removeItem("waterOpsShift");
...
async function closeCurrentShift(){
  ...
  const res=await fetch("/api/mobile/shift/close",{method:"POST",headers:authHeaders({"content-type":"application/json"}),body:JSON.stringify({shiftId:currentShift.id, shift: currentShift, role: appRole, employeeName: appEmployee})});
  const data=await res.json();
  if(res.ok){
    localStorage.removeItem("waterOpsShift");
...
$("auditButton").onclick=async()=>{
  ...
  const res=await fetch("/api/mobile/audit?from="+encodeURIComponent(from)+"&to="+encodeURIComponent(to),{headers:authHeaders()});
  const data=await res.json();
  if(!res.ok){
...
async function saveReportEdits(){
  ...
  for(const row of rows){
    ...
    if(row.dataset.delete === "1"){
      const res = await fetch("/api/mobile/sales/"+encodeURIComponent(id), { method: "DELETE", headers:authHeaders() });
...
async function loadWarehouseDebt(){
  ...
  const res = await fetch("/api/mobile/warehouse/debt?"+params.toString(),{headers:authHeaders()});
  const data = await res.json();
  ...
  return '<div class="report-table">'+
    ...
    rows.map((row)=>
      '<div class="report-row" data-sale-id="'+escapeHtml(row.id || '')+'">'+
        '<b>'+escapeHtml(row.destination)+'<small>'+escapeHtml(row.employeeName || "Старые данные")+' · '+escapeHtml(row.dateTime || row.date || "")+'</small></b>'+
  ...
  '<div class="report-table client-table">'+
    ...
    rows.map((row,index)=>
      '<div class="report-row '+(index >= 5 ? 'client-extra hidden' : '')+'">'+
        '<b>'+escapeHtml(row.name)+'<small>'+escapeHtml(row.employeeName || "Старые данные")+' · '+escapeHtml((row.saleTimes || []).slice(0,4).join(", ") || "дата не указана")+'</small></b>'+
